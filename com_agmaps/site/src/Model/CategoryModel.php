...
class CategoryModel extends ListModel
{
	...
	/**
	 * Array of agmaps in the category
	 *
	 * @var    \stdClass[]
	 */
	...
	/**
	 * The list of other agmap categories.
	 *
	 * @var    array
	 */
	...
	public function getItems()
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			if ($this->getState('load_tags', true))
			{
				...
				$this->tags->getItemTags('com_agmaps.agmap', $item->id);
			}
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$numberOfAgmapsToDisplay = $mergedParams->get('agmaps_display_num');

		if ($format === 'feed')
		{
			$limit = $app->get('feed_limit');
		}
		elseif (isset($numberOfAgmapsToDisplay))
		{
			$limit = $numberOfAgmapsToDisplay;
		}
		else
		{
		...
		$search = $app->getUserStateFromRequest('com_agmaps.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
		...
		if ((!$user->authorise('core.edit.state', 'com_agmaps')) && (!$user->authorise('core.edit', 'com_agmaps')))
		{
	...
	public function getCategory()
	{
		if (!is_object($this->_item))
		{
			...
			$categories = Categories::getInstance('Agmaps', $options);
