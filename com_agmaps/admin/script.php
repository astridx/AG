...
/**
 * Script file of Agmap Component
 *
 * @since  __BUMP_VERSION__
 */
class Com_AgmapsInstallerScript extends InstallerScript
{
	/**
	...
	{
		echo Text::_('COM_AGMAPS_INSTALLERSCRIPT_INSTALL');

		$db = Factory::getDbo();
		$alias   = ApplicationHelper::stringURLSafe('AgmapUncategorised');

		// Initialize a new category.
		$category = Table::getInstance('Category');

		$data = array(
			'extension' => 'com_agmaps',
			'title' => 'AgmapUncategorised',
			'alias' => $alias . '(en-GB)',
			'description' => '',
			'published' => 1,
			'access' => 1,
		...
		$this->addDashboardMenu('agmaps', 'agmaps');
	...
	{
		echo Text::_('COM_AGMAPS_INSTALLERSCRIPT_UNINSTALL');
	...
	{
		echo Text::_('COM_AGMAPS_INSTALLERSCRIPT_UPDATE');

		$this->addDashboardMenu('agmap', 'agmap');
	...
	public function preflight($type, $parent): bool
	{
		...
		echo Text::_('COM_AGMAPS_INSTALLERSCRIPT_PREFLIGHT');
	...
	{
		echo Text::_('COM_AGMAPS_INSTALLERSCRIPT_POSTFLIGHT');
	...
	private function saveContentTypes()
	{
		...
		$table->load(array('type_alias' => 'com_agmaps.agmap'));
		...
		$tablestring = '{
			"special": {
			  ...
			  "key": "id",
			  "type": "AgmapTable",
			  "prefix": "Joomla\\\\Component\\\\Agmaps\\\\Administrator\\\\Table\\\\",
			  "config": "array()"
			},
			"common": {
		...
		$contenttype['type_title'] = 'Agmaps';
		$contenttype['type_alias'] = 'com_agmaps.agmap';
		$contenttype['table'] = $tablestring;
		$contenttype['rules'] = '';
		$contenttype['router'] = 'RouteHelper::getAgmapRoute';
